...
The commands for generation used to achieve the results shown in the project are `python -m models.lstm.generate env/training/len500 -b 10 -l 500` and `python -m models.lstm.generate env/training/len1500 -b 10 -l 1500`.

#### Training the models

The python script at `ews/lstm/training/train.py` can be run in order to train models. It takes in a path argument defining the directory of the training data generated from previous runs of the generator script. This input directory can read recursively for multiple runs of data, such as over multiple time-series lengths. It also takes in another path argument defining the output directory to place the completed deep learning models in.

The command for training the models used to achieve the results shown in the project are `python -m models.lstm.train env/training/len$TS_LEN/ -o models/lstm/len$TS_LEN/ -n best_model_$N` where TS_LEN is the time series length from the generated data and N is the nth model trained.
